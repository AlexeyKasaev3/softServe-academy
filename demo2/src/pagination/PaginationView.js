...
export class PaginationView {
  constructor(navClickHandler) {
    ...
    this.paginationButtonsDOMparent = document.querySelector(
      ".pagination-list"
    );

    this.paginationDOMparent.addEventListener(
      "click",
      this.routePaginationPanelClick.bind(this)
    );
    ...
    if (totalPagesQuantity < 2) {
      ...
      if (this.currentPage === 1) {
        document
          ...
          .classList.add("out-of-rich");
        document
          .querySelector(".pagination-next")
          .classList.remove("out-of-rich");
      ...
      if (this.currentPage + 2 < this.totalPagesQuantity - 1) {
        buttons += this.template.getPaginationDots();
        buttons += this.template.getPaginationButton(
          this.totalPagesQuantity,
          false
        );
      }
